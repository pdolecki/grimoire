...
@Component({
  ...
  template: `
    <section class="flashcards" aria-labelledby="flashcards-heading">
      ...
      <!-- Sticky toolbar (great on desktop; stays visible as you scroll) -->
      <div class="toolbar sticky" role="group" aria-label="Selection controls">
      ...
      <!-- Mobile bottom action bar (appears only when something is selected) -->
      @if (anySelected()) {
      <div class="mobile-cta" role="group" aria-label="Selected actions">
        ...
        <button type="button" class="btn primary" (click)="startLearning()">
          Start
        </button>
      </div>
      }
  ...
  styles: [
    `
      ...
      .mobile-cta .mobile-count {
        font-weight: 600;
        color: var(--cl-light-06);
      }

      /* Show the mobile bar only on small screens */
...
export default class Flashcards {
  ...
  constructor() {
    ...
    if (raw) {
      try {
        this.selected.set(new Set(JSON.parse(raw)));
      } catch {
        // ignore bad storage
      }
    }
